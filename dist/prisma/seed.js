...
async function main() {
    ...
    const order2 = await prisma.order.create({
        data: {
            ...
            status: client_1.OrderStatus.STITCHING,
    ...
    const order3 = await prisma.order.create({
        data: {
            ...
            remainingAmount: 0.0,
            status: client_1.OrderStatus.CUTTING,
    ...
    await prisma.orderTimeline.createMany({
        data: [
            { orderId: order1.id, previousStatus: null, newStatus: client_1.OrderStatus.PENDING, updatedBy: adminUser.id, createdAt: new Date(Date.now() - 10 * 24 * 60 * 60 * 1000) },
            { orderId: order1.id, previousStatus: client_1.OrderStatus.PENDING, newStatus: client_1.OrderStatus.MEASURING, updatedBy: recepUser.id, createdAt: new Date(Date.now() - 9 * 24 * 60 * 60 * 1000) },
            { orderId: order1.id, previousStatus: client_1.OrderStatus.MEASURING, newStatus: client_1.OrderStatus.CUTTING, updatedBy: tailorUser.id, createdAt: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) },
            { orderId: order1.id, previousStatus: client_1.OrderStatus.CUTTING, newStatus: client_1.OrderStatus.STITCHING, updatedBy: tailorUser.id, createdAt: new Date(Date.now() - 6 * 24 * 60 * 60 * 1000) },
            { orderId: order1.id, previousStatus: client_1.OrderStatus.STITCHING, newStatus: client_1.OrderStatus.QUALITY_CHECK, updatedBy: managerUser.id, createdAt: new Date(Date.now() - 4 * 24 * 60 * 60 * 1000) },
            { orderId: order1.id, previousStatus: client_1.OrderStatus.QUALITY_CHECK, newStatus: client_1.OrderStatus.READY, updatedBy: managerUser.id, createdAt: new Date(Date.now() - 4 * 24 * 60 * 60 * 1000) },
            { orderId: order1.id, previousStatus: client_1.OrderStatus.READY, newStatus: client_1.OrderStatus.DELIVERED, updatedBy: cashierUser.id, createdAt: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000) },
        ],
    });
    // Order 2 Timeline
    await prisma.orderTimeline.createMany({
        data: [
            { orderId: order2.id, previousStatus: null, newStatus: client_1.OrderStatus.PENDING, updatedBy: recepUser.id, createdAt: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000) },
            { orderId: order2.id, previousStatus: client_1.OrderStatus.PENDING, newStatus: client_1.OrderStatus.STITCHING, updatedBy: managerUser.id, createdAt: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000) },
        ],
    });
    ...
    await prisma.payment.createMany({
        data: [
            { orderId: order1.id, amount: 400.0, paymentMethod: client_1.PaymentMethod.CASH, notes: 'Advance Deposit', createdAt: new Date(Date.now() - 10 * 24 * 60 * 60 * 1000) },
            { orderId: order1.id, amount: 450.0, paymentMethod: client_1.PaymentMethod.CARD, notes: 'Final settlement on delivery', createdAt: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000) },
        ],
    });
    ...
    await prisma.payment.create({
        data: {
            orderId: order2.id,
            amount: 200.0,
            paymentMethod: client_1.PaymentMethod.EASYPAISA,
    ...
    await prisma.payment.create({
        data: {
            orderId: order3.id,
            amount: 300.0,
            paymentMethod: client_1.PaymentMethod.BANK,
